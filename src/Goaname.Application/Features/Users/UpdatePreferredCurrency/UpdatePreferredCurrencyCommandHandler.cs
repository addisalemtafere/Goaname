...
public sealed class UpdatePreferredCurrencyCommandHandler(IGrainFactory grainFactory, ISender sender)
    : IRequestHandler<UpdatePreferredCurrencyCommand, UserProfileDto>
{
    public async Task<UserProfileDto> Handle(UpdatePreferredCurrencyCommand request, CancellationToken cancellationToken)
    {
        ...
        await userGrain.UpdatePreferredCurrencyAsync(request.Currency).ConfigureAwait(false);
