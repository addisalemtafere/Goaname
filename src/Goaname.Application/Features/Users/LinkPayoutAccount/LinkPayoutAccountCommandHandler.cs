...
public sealed class LinkPayoutAccountCommandHandler(IGrainFactory grainFactory, ISender sender)
    : IRequestHandler<LinkPayoutAccountCommand, UserProfileDto>
{
    public async Task<UserProfileDto> Handle(LinkPayoutAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        await userGrain.LinkPayoutAccountAsync(request.Provider, request.AccountId).ConfigureAwait(false);
